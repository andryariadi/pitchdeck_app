...
const Button: FC<ButtonProps> = ({ btn, title, icon: Icon, url }) => {
  return btn ? (
    ...
    >
      <div className="flex items-center justify-center w-full transition-all duration-300 md:group-hover:justify-start md:group-hover:px-3">
        <Icon size={20} className="text-primary" />
      </div>
      <div className="absolute right-2 md:transform md:translate-x-full opacity-0 text-primary text-sm font-semibold md:group-hover:translate-x-0 md:group-hover:opacity-100">{title}</div>
    </Link>
  );
};
